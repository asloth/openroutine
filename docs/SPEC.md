...
Design tokens live once in `app/lib/theme/`. **The design pass is done** — the tokens come from the "FocusFlow Routine Timer" Stitch project, and its export carries a full Material 3 role set, so `theme/colors.dart` is a literal `ColorScheme` rather than an approximation from a seed. Primary is `#0051c0`, secondary is `#006c47`, and surface is `#f7f9fc`; Bricolage Grotesque handles structure and actions, Plus Jakarta Sans handles prose, and both ship as static weights (never fetched at runtime — see §1); radii are 4/8/12/full; spacing is 8/16/24/32 with a 48px minimum touch target. Neumorphic surfaces (paired light and dark shadows) are a `ThemeExtension`, so they adapt to brightness.
