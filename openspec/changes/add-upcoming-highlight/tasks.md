...
- [x] 1.1 Extract `ReminderSchedule._parseStartTime` and `._weekday` into a new
...
- [x] 2.1 Add `test/services/routines/estimate_test.dart` covering: sums timed steps, skips
      `noExplicitTime` steps, treats a null duration as zero, and returns zero for an empty
      list; verify it fails (no such file yet)
- [x] 2.2 Add `app/lib/services/routines/estimate.dart` with `routineEstimate`, and verify 2.1
      passes

## 3. Upcoming-state service (slice 3)

- [x] 3.1 Add `test/services/routines/upcoming_test.dart` covering all 16 cases from
      `specs/upcoming-routines/spec.md`, and verify it fails
- [x] 3.2 Add `app/lib/services/routines/upcoming.dart` with the `UpcomingState` sealed type
      (`StartsIn`, `InProgress`) and `upcomingState(...)`, reusing `ScheduleTime`, and verify
      3.1 passes

## 4. Clock provider (slice 4)

- [x] 4.1 Add `app/lib/state/clock_provider.dart`: a `@Riverpod(keepAlive: true)` `Clock`
...
- [x] 4.2 Run codegen (`dart run build_runner build --delete-conflicting-outputs`)

## 5. ARB strings (slice 5)

- [x] 5.1 Add `routinesUpcomingIn` ("in {minutes} min", int placeholder, with a description)
      and `routinesUpcomingNow` ("Now") to `app_en.arb`
- [x] 5.2 Add the Spanish translations ("en {minutes} min", "Ahora") to `app_es.arb`
- [x] 5.3 Run `flutter gen-l10n`

## 6. Wire the card (slice 6)

- [x] 6.1 Add widget-test coverage in `routines_list_screen_test.dart` (clock overridden): a
...
- [x] 6.2 In `_RoutineCard`, watch `clockProvider` and `routineCompletionsProvider(routine.id)`
