...
void main() {
  ...
  // The recipe's contrast contract has to hold at any hue, not just the six
  // shipped built-ins, because every built-in but warm paper is generated by
  // it — this is what would catch the recipe drifting for a hue no built-in
  // happens to sit on.
  group('generated palettes', () {
    for (var hue = 0; hue < 360; hue += 10) {
      test('hue $hue° meets AA in both themes', () {
        checkPalette(
          Palette.fromSeed(id: 'hue_$hue', hue: hue.toDouble()),
          'hue $hue',
        );
      });
    }
  ...
  group('identity', () {
    test('palettes with the same id are equal', () {
      final a = Palette.fromSeed(id: 'test_hue', hue: 268);
      final b = Palette.fromSeed(id: 'test_hue', hue: 268);
      expect(a, b);
